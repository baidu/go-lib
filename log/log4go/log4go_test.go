...
func newLogRecord(lvl LevelType, src string, msg string, data []byte) *LogRecord {
	return &LogRecord{
		Level:   lvl,
		Source:  src,
		Created: now,
...
func TestELog(t *testing.T) {
	...
	lr := newLogRecord(CRITICAL, "source", "message", nil)
...
func TestFileLogWriter(t *testing.T) {
	...
	w.LogWrite(newLogRecord(CRITICAL, "source", "message", nil))
	w.Close()
	runtime.Gosched()
...
func TestTimeFileLogWriter(t *testing.T) {
	defer func(buflen int) {
		LogBufferLength = buflen
	}(LogBufferLength)
	LogBufferLength = 0
...
func TestTimeFileLogWriter_Binary(t *testing.T) {
	defer func(buflen int) {
		LogBufferLength = buflen
	}(LogBufferLength)
	LogBufferLength = 0
